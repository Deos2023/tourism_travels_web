...
const TourCard = ({ tour }) => (
  <div className="flex gap-6 backdrop-blur-md sm:ml-30 shadow-2xl p-6 relative z-20">
    ...
    <div className="flex-1 text-white">
      ...
      <p className="text-sm text-white/80">{tour.rating} Superb</p>
      <p className="text-white/90 text-sm my-2 line-clamp-4">{tour.description}</p>
      <p className="text-xl font-semibold text-yellow-300">
        ${tour.price}{' '}
        <span className="text-sm font-normal text-white/70">/ per person</span>
      </p>
      <div className="flex flex-wrap gap-2 mt-3 text-xs font-medium">
        {Array.isArray(tour.tags) &&
          tour.tags.map((tag, i) => (
            <span key={i} className="bg-orange-600 px-3 py-1 rounded-full">
              {tag}
            </span>
          ))}
      </div>
  ...
  return (
    <div className="p-6 min-h-screen relative py-20">
      ...
      <div className="fixed inset-0 -z-10">
        <div 
          className="absolute inset-0 bg-[url('/bgStatic.webp')] bg-cover bg-center bg-no-repeat"
        />
        <div className="absolute inset-0 bg-yellow-400/40 opacity-80" />
      </div>
      ...
      <div className="container mx-auto px-4 relative">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-white/90 mb-8 max-w-2xl mx-auto">
            Explore breathtaking destinations with our curated tour packages
          </p>
        ...
        <div className="flex-1">
          <div className="flex flex-col gap-8">
            {visibleTours.length > 0 ? (
              ...
              <div className="bg-white/50 backdrop-blur-md p-6 rounded-lg shadow-md text-center text-white">
